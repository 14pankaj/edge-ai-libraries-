...
if [[ ${SKIP_BACKEND} -eq 0 ]]; then
  require_cmd docker
  require_cmd curl
  # Skip startup entirely if the backend is already responding to avoid restarting a service managed elsewhere.
  if curl -fsS "${HEALTH_URL}" >/dev/null 2>&1; then
    log "Backend already healthy at ${HEALTH_URL}, skipping startup"
    SKIP_BACKEND=1
  else
